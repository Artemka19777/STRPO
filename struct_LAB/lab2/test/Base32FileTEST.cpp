...
int main() {
    const char* filename = "data/test_base32_temp.txt";
    ...
    {
        Base32File reader1(filename, "w+");
        size_t totalWritten = 0;
        
        while (totalWritten < DATA_SIZE) {            
            size_t written = reader1.write(originalData + totalWritten, 256);
