...
// Для проверки move семантики 1.3
// void* operator new[](size_t size) {
//     cout << "Global Alloc: " << size << " bytes" << endl;
//     return malloc(size);
// }
...
void printSimple(const BoolArray& arr) {
    cout << "[";
...
int main() {
    ...
    // перегрузил оператор присваивания еще в прошлых работах, из-за проблем с двойным освобождением памяти
     ...
     */
    /*Методом класса нельзя перегрузить оператор *, т.к. при перегрузке в качестве метода класса, компилятор будет ожидать,
    что слева будет стоять объект Matrix(this)? потому что это метод класса, а справа от =, он ожидает параметр double*/
    ...
    */
    ...
    {
        MyString s1("dsfs");
        MyString s2= addTxtExtension(s1);
        s2.print();
    }
    /**
     ...
     */
    {
        ...
        // Заполняем базу
        db["Ivanov"] = WorkerData("Ivan", 37);
        db["Petrov"] = WorkerData("Petr", 43);
        db["Sidorov"] = WorkerData("Sidor", 20);
        print_db(db);
        cout << "AVG: " << get_avg_age(db) <<'\n';
    }
    /**
     ...
     */
    {
        ...
        notifications[0] = createSystem(1001, "Low battery", Attetion::Normal);
        notifications[1] = createMessege(1005, "Mom", "Call me");
        notifications[2] = createApp(1010, "Telegram", "Friend", "HELLO");
        notifications[3] = createSystem(321015, "ERROR", Attetion::SOS);

        for (int i = 0; i < SIZE; ++i) {
            notifications[i].print();
        }

        int systemCount = countType(notifications, SIZE, NotType::System);
        cout << "\nNotifications: " << systemCount << '\n';
     ...
     */
    /*С помощью наследования я бы создал абстрактный класс с чисто виртуальными методами и вместо структур создал бы производные классы.
    Мне кажется, что главный недостаток union, это то, что нужно вручную првоерять тип уведомления, тогда как при наследовании это бы не пришлось делать,
    также нет проблем с деструкторами, они вызываются в правильном порядке, также легче поддерживать и расширять код.

    Из минусов - это занимает больше памяти, т.к. в union хранится один объект, плюс нужна таблица и указатели для виртуальн
    ых методов. Также нужно создавать массив через указатель, т.к. структуры могут занимать разный объем памяти. */
    /**
     ...
     */
    {
        ...
        q.push(createSystem(198001, "Low Battery", Attetion::Normal));
        q.push(createMessege(165005, "Mom", "Call me!"));
        q.push(createApp(106510, "Telegram", "News", "Hello world"));

        cout << "size: " << q.size()<<"\n";

        std::cout << "All notifications in queue:\n";
        for (Notification* it = q.begin(); it != q.end(); it++) {
            it->print();
        }

        // Тестируем pop()
        cout << "\nfirst element:\n";
        Notification first = q.pop();
        first.print();
     ...
     * 
     * Явно оператор присваивания не определен, он определен компилятором, который просто копирует все поля. Если же самому определять оператор,
     * то непонятно, как хранить указатль, будет double free, даже если как-то реализовать это, то возникнет проблема, в том, что в скопированном файле
     * указатель чтения будет смещаться в начало, поэтому оператор не имеет смысла
     *  
     *
    ...
    {
        BaseFile f1("data/test.txt", "w");
    
        // BaseFile f2 = f1; //конструктор копирования удален
        // f2 = f1 //оператор присваивания удален
        BaseFile f3 = std::move(f1); 
