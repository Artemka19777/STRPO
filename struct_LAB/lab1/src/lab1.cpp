...
int fibonacci(int n) {
    ...
    return static_cast<int> (result.get(0,0));
}
...
int main()
{
    ...
    /*
    1)Переменные заполнены случайными, мусорными значениями, т.к. переменные локальные
    //2)Вызывается конструктор, но т.к. он не определен, компилятор создает конструктор по умолчанию
    //3)В этом случае вызывается неявный конструктор по умолчанию, т.к. я его не объявляли для встроенный типов поля не инициализируются
     В общем случае конструктор создается, если программист его не объявил, для встроенных типов -мусор, для других классов - вызываются их конструкторы
    */
    {
        Rect rect;
    } 
     ...
     * нулями.
     */
    /*ошибка компиляции пояивлась из-за того, что мы уже прописали конструктор(он принимает параметры, которые мы не указали), и консутрктор по умолчанию больше не создается.
    Конструктора без параметров больше нет
    */
    {
        Rect rect(1, 2, 3, 4);
    } 
    ...
    {
        Rect rect1(1, 2, 3, 4); //вызывается конструктор с параметром (определил пользователь)
        Rect rect2 = rect1;//конструктор копирования(определил комиплятор) в этом случае, если пользователь создал, то вызывается пользовательский конструктор
        Rect rect3(rect1);//компилятор ищет консутруктор Rect(const Rect& other), если пользователь его не создавал,
        //то компилятор сам его создает и производит поэлементное копирование
    } 
    ...
    Деструктор вызывается автоматически, когда объект выходит из области видимости, также когда объект был создан через указатель.
        В моем случае деструктор не прописан, поэтому компилятор создает его автоматически, деструктор освобождает память, поля удаляются поэлементно
        В общем случае деструктор осовбождает поля встроенных тип и вызывает деструкторы, если поля являются классами.
    Если поле - указатель на динамическую память, то при уничтожении объекта память не помечается как свободная.
    Деструктор вызывается, при выходе из блока
*/
...

    {
        ...
        Rect *pR = new Rect(1,2,1,2); //конструктор с параматерами(динамический)
        {
            Rect r2(r1);//конструктор копирования 
            Rect arRect[2];//два конструктора по умолчанию 
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i);// один конструктор по умолчанию при первом проходе, т.к. в статической памяти 
                Rect r4(*pR);// конструктор копирования 3 раза
                Rect r5(i,i,i,i);//конструктор с параметрам 3 раза 
            }//после каждой итерации деструктор r4, r5
        }// деструктор r2 arRect (2 элемента)
        delete pR;//деструктор для динамического, т.к. сам он не осовбодит память  
    }//r1 деструктор, r3 до конца работы программы 

    /*ИТОГ: 
        r1(1)+pr(1)+r2(1)+arRect(2)+r3(1)+r4(3)+r5(3) = 12 - КОНСТРУКТОРЫ 
        r4(3)+r5(3)+r2(1)+arRect(2)+pr(1)+r1(1)+r3(1) = 12 - ДЕСТРУКТОРЫ
    */
    ...
    //     cout<< r1.left<< endl;
    // } 
    // private поля нельзя использовать напрямую вне методов класса
     ...
     */
    /*
       Метод является частью класса и имеет доступ к его  private полям.
       При вызове метода неявно передаётся указатель this на объект, для которого вызывается метод.
       Обычная функция существует вне зависимости от объектов, а метод привязан к определнному объекту.
    */
    ...
    {
        Rect r1(2,6,7,3), r2(5,10,15,-2);//конструктор с параметрами 
        Rect r3 = bounding_rect(r1, r2);//конструктор копирования для r1, r2, также копирование результата функции в r3
        print_rect(r3);
    } 
    ...
    {
        Rect r1(2,8,13,4);
        cout<< r1.get_width()<<" "<< r1.get_height()<<" "<< r1.get_square()<<'\n'; 
        r1.set_width(2);
        r1.set_height(20);
        cout<< r1.get_width()<<" " << r1.get_height() <<" "<<r1.get_square(); 
    }
     ...
     */
    //при изменении пересобирается только нужный .o файл
    /**
     ...
     */
     ...
     /*
     1)создал переменную object со всеми объектными файлами
     2)добавил цель clean, которая не имеет зависимостей, для отчистки объектных файлов и исполняемого файла.
     добавил флаг -f, т.к. может быть 2 варианта исполняемого файла
